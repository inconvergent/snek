  ...
  (let* ((itt 500000))
    ...
    (time
      (loop for i from 0 below itt
            do (lin-path:make (rnd:nin-box 10 500d0 500d0 :xy (vec:vec 500d0)))))

    (format t "make 100")
    (time
      (loop for i from 0 below itt
            do (lin-path:make (rnd:nin-box 100 500d0 500d0 :xy (vec:vec 500d0)))))

    (format t "pos 10")
    (time
      (loop with pos = (rnd:nrnd 100)
            with lp = (lin-path:make (rnd:nin-box 10 500d0 500d0 :xy (vec:vec 500d0)))
            for i from 0 below itt
            do (lin-path:pos* lp pos)))
